WITH
     ...
     (
         SELECT
                DISTINCT
                ifd.ifd_vertical_fine_item_id,
                LAST_VALUE(item.pure_item_id)        OVER (PARTITION BY ifd.ifd_vertical_fine_item_id
                                                               ORDER BY ifd.ifd_vertical_index) AS pure_item_id,
                LAST_VALUE(ifd.ifd_vertical_index) OVER (PARTITION BY ifd.ifd_vertical_fine_item_id
                                                               ORDER BY ifd.ifd_vertical_index) AS ifd_index
