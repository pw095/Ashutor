WITH
     ...
     (
         SELECT
                DISTINCT
                ifc.ifc_horizontal_fine_item_id,
                LAST_VALUE(item.pure_item_id)        OVER (PARTITION BY ifc.ifc_horizontal_fine_item_id
                                                               ORDER BY ifc.ifc_horizontal_index) AS pure_item_id,
                LAST_VALUE(ifc.ifc_horizontal_index) OVER (PARTITION BY ifc.ifc_horizontal_fine_item_id
                                                               ORDER BY ifc.ifc_horizontal_index) AS ifc_index
